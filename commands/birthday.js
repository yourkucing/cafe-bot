        ...
        if (!birthdaykids || birthdaykids.length == 0) {
            ...
            if (birthdaykids.length > 10) {
                ...
                for (let i = 0; i < birthdaykids.length; i += 10) {
                    ...
                    for (m in current) {
                        ...
                        if (!users) {
                            ...
                            birthdays += `**${++j}.**  **${users.username}**: ${current[m].birthday.getDate()} ${current[m].birthday.toLocaleString('default', { month: 'long' })}\n`
                        }
                    }
                ...
                for (x in birthdaykids) {
                    ...
                    birthdaylist += `**${n}.** **${users.username}**: ${birthdaykids[x].birthday.getDate()} ${birthdaykids[x].birthday.toLocaleString('default', { month: 'long' })}\n`
                    n++
                }
